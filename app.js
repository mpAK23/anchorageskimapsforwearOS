...
function styleTrail(feature) {
    ...
    const skiOnly = p.Ski_Only; // e.g., 'Yes' or 'No'

    let color = '#ecf0f1'; // Default white
    let weight = 3;
    let dashArray = null;

    // Distinguish multi-use vs ski-only using dashArray
    if (skiOnly === 'No' || skiOnly === 'N') {
        dashArray = '5, 10'; // dashes for multi-use trails
    }
